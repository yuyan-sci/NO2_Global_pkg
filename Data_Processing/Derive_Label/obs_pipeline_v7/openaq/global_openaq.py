...
def convert_units_to_ppb(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if units_col is None:
        df["value"] = df["value"].apply(lambda v: v * 1000 if v < 1 else v)
        df["units"] = "ppb"
        return df

    def to_ppb(row):
        unit = str(row[units_col]).lower()
        if unit == "ppm":
            return row["value"] * 1000, "ppb"
        elif unit == "ppb":
            return row["value"], "ppb"
        elif unit == "µg/m³":
            return row["value"] * 24.45 / 46, "ppb"
...
def process_country_year(
    ...
    # Fixed delay to avoid hitting global rate limits
    # OpenAQ free tier often limits to ~60 requests/minute (1/s).
    # We'll use 1.5s to be safe.
    delay = 1.5
    print(f"    Fixed delay set to {delay:.2f}s for {num_sensors} sensors.")
    sys.stdout.flush()
