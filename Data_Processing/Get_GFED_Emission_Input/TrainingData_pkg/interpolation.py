...
def Bilinearinterpolate_GC_concentraion(Cxfyf:np.array,Cxfyc:np.array,Cxcyf:np.array,Cxcyc:np.array,delta_x:float,delta_y:float,dx:np.array,dy:np.array):
    ...
    x_weight = np.clip(dx/delta_x, 0, 1)
    y_weight = np.clip(dy/delta_y, 0, 1)
    
    # Handle NaN values
    Cxfyf = np.nan_to_num(Cxfyf, nan=0.0)
    Cxfyc = np.nan_to_num(Cxfyc, nan=0.0)
    Cxcyf = np.nan_to_num(Cxcyf, nan=0.0)
    Cxcyc = np.nan_to_num(Cxcyc, nan=0.0)
    
    Cx1 = Cxcyf * x_weight + Cxfyf * (1-x_weight)
    Cx2 = Cxcyc * x_weight + Cxfyc * (1-x_weight)
    Concentration = Cx2 * y_weight + Cx1 * (1-y_weight)
