...
! For 0.01x0.01
  ...
  DO i=1,nval
     DO j=1, nlat
        WRITE (OUTLUN, '(36001D14.5)') gridmean(i, j, 1:nlon)
     ENDDO
  ENDDO

  DO j = 1, nlat
     WRITE (OUTLUN, '(36001D14.5)') totarea(j, 1:nlon)
  ENDDO

  DO j = 1, nlat
     WRITE (OUTLUN, '(36001I5)')    INT(narea(j, 1:nlon))
  ENDDO
  CLOSE (OUTLUN)
